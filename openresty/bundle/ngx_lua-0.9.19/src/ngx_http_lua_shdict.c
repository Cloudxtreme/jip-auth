...
static int ngx_http_lua_shdict_set_expire(lua_State *L);
...
static int ngx_http_lua_shdict_get_values(lua_State *L);
static int ngx_http_lua_shdict_flush_keys(lua_State *L);
static int ngx_http_lua_shdict_get_count(lua_State *L);
...
ngx_http_lua_inject_shdict_api(ngx_http_lua_main_conf_t *lmcf, lua_State *L)
{
    ...
    if (lmcf->shm_zones != NULL) {
        ...
        lua_createtable(L, 0 /* narr */, 17 /* nrec */); /* shared mt */
        ...
        lua_pushcfunction(L, ngx_http_lua_shdict_set_expire);
        lua_setfield(L, -2, "set_expire");
        ...
        lua_pushcfunction(L, ngx_http_lua_shdict_get_values);
        lua_setfield(L, -2, "get_values");

        lua_pushcfunction(L, ngx_http_lua_shdict_flush_keys);
        lua_setfield(L, -2, "flush_keys");
		
        lua_pushcfunction(L, ngx_http_lua_shdict_get_count);
...
static int
ngx_http_lua_shdict_get_values(lua_State *L)
{
    ngx_queue_t                 *q, *prev;
    ngx_http_lua_shdict_node_t  *sd;
    ngx_http_lua_shdict_ctx_t   *ctx;
...
static int
ngx_http_lua_shdict_flush_keys(lua_State *L)
{
    ngx_queue_t                 *q, *prev;
    ngx_http_lua_shdict_node_t  *sd;
    ngx_http_lua_shdict_ctx_t   *ctx;
    ...
    checktime = (uint64_t) tp->sec * 1000 + tp->msec
    ...
    while (q != ngx_queue_sentinel(&ctx->sh->queue)) {
        ...
        dd("node expires: %lld", (long long) sd->expires);
        dd("time to live: %lld", (long long) (sd->expires - checktime));

        if (sd->expires < checktime) {
            total++;
            if (attempts && total == attempts) {
                break;
            }
...
/*
 * This trades CPU for memory. This is potentially slow. O(n)
 */

static int
ngx_http_lua_shdict_get_count(lua_State *L)
{
    ngx_queue_t                 *q, *prev;
    ngx_http_lua_shdict_node_t  *sd;
    ngx_http_lua_shdict_ctx_t   *ctx;
    ...
    /* first run through: get total number of elements we need to compute */
...
allocated:
    ...
    dd("setting value type to %d", value_type);
    dd("node expires: %lld", (long long) sd->expires);
    dd("time to live: %lld", (long long) (exptime * 1000));
...
static int
ngx_http_lua_shdict_set_expire(lua_State *L)
{
    int                          n;
    ngx_str_t                    key;
    uint32_t                     hash;
